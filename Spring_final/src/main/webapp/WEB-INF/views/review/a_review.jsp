...
<div class="modal" >
	...
	<input type="hidden" name="ar_no" value="${i.getAr_no() }">
	
	<tr align="left">
		<td> ${i.getUser_id()}&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;${i.getAr_date() }</td>
		
		<td colspan="2" align="right">
			<input type="button" class="btn btn-outline-danger btn-sm " value="삭제"
				onclick="if(confirm('삭제하시겠습니까?')) {
					location.href='arlist_delete.do?no=${i.getAr_no()}'
				}else { return; }">
		</td>
	</tr>
	<tr align="left">
	...
	<tr align="left">
		...
		<c:if test="${ !empty i.getAr_img() }">
				<td colspan="4" align="right"> <img width="130" height="100" src="<%=request.getContextPath()%>/resources/image/${i.getAr_upload()}"> </td>
		</c:if>
		<c:if test="${ empty i.getAr_img() }">
				<td> ${i.getAr_img() } </td>
